...
<div class="frm-control">
    <h3>Danh sách Khách đặt tour theo yêu cầu</h3>
    <?php
    $keyword=''; $action='';
    ...
    if($keyword!='' && $keyword!='Keyword')
        $strwhere.="AND `tbl_tour`.`name` like '%$keyword%' OR `tbl_tour_person`.`fullname` like '%$keyword%'";
    if($action!='' && $action!='all' ){
        $action=(int)$action;
       // $strwhere.="WHERE `tbl_tour_person`.`isactive` = '$action'";
    }
    $limit='';
        ...
        <form id="frm_list" name="frm_list" method="post" action="">
            <div class='row'>
					...
					<div class="form-inline">
					   <label class="sr-only" for="exampleInputAmount">Tìm kiếm</label>
						<div class="input-group">
						 ...
						 <div class="input-group-addon"><input type="submit" name="button" id="button" value="Search" class="button btn-primary" /></div>
						</div>
					</div>
				</div>
               <!-- <div class='col-lg-6 col-md-6 box-selected text-right'>
                    ...
                    <a href="<?php /*echo ROOTHOST;*/?>member/tour-type/them-moi" class="add-new btn btn-success pull-right">Thêm mới</a>
                </div>-->
            </div>
...
<script>
    $('.show-detail').click(function(){
        ...
        $.get('<?php echo ROOTHOST;?>ajaxs/tour/showDetailTourRequest.php', {value}, function(response_data){
            $('#myModal').modal('show');
            $('#myModalLabel').html('Thông tin khách hàng đặt tour');
