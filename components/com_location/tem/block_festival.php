		...
		<div class="container">
			<div class="box-path">
				<ul class="list-inline">
					...
					> <li><a href="#" class="active">Lễ hội</a></li>
				</ul>
			</div>
			<?php
			$strWhere="WHERE `tbl_location_content_festival`.`location_id`='".$location_id."'";
			$sql="SELECT count(*) AS 'num' FROM tbl_location_content_festival $strWhere AND isactive=1";
			$objdata->Query($sql);
			...
			if($total_rows>0){
				echo ' <div class="row">';
				$sql="SELECT `id`, `code`, `thumb`, `cdate`,`name`, `intro`, `author` FROM `tbl_location_content_festival` ".$strWhere." ORDER BY `cdate` DESC LIMIT $start,$max_rows ";
				$objdata->Query($sql);
				while($rows=$objdata->Fetch_Assoc()){
					$intro=strip_tags(Substring($rows['intro'], 0, 20));
					$url=ROOTHOST.$location_code."/le-hoi/".$rows['code'].".html";
