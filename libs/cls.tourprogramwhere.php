...
class CLS_TOURPROGRAMWHERE{
    ...
    public function getAvatarAndNameByPositionContactId($positionId){
        $sql="SELECT `tbl_position`.`id`, `tbl_position`.`name`, `tbl_position`.`avatar`, `tbl_position`.`address`,
              `tbl_location`.`code` as `location_code`,`tbl_position`.`code` as `position_code` FROM `tbl_position` INNER JOIN `tbl_location`
              ON `tbl_position`.`location_id`=`tbl_location`.`id` WHERE `tbl_position`.`id`=$positionId";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
            ...
            while($rows=$objdata->Fetch_Assoc()){
                ...
                <div class="item">
                    ...
                    <div class="list_img">
                        <div class="row">
                        <?php
                            $sql1="SELECT `tbl_gallery`.`arr_path` FROM `tbl_position` LEFT JOIN `tbl_gallery`
                                    ON `tbl_position`.`id`=`tbl_gallery`.`par_id` WHERE `tbl_position`.`code`='".$position_code."'
                                    AND `tbl_gallery`.`arr_path` IS NOT NULL AND `tbl_gallery`.`type`='1'";
