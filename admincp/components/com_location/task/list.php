...
$sql="SELECT count(*)  as num FROM `tbl_location` WHERE 1=1 ".$strwhere;
$objdata->Query($sql);
...
<div id="list">
    <form id="frm_list" name="frm_list" method="post" action="">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="Header_list">
            <tr>
                ...
                <td align="right">
                    <a href="?com=location&task=add" class="btn btn-success button"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
                </td>
            </tr>
        </table>
        ...
        <table class="table table-bordered">
            ...
            $leng=MAX_ROWS;
            $sql="SELECT * FROM `tbl_location` WHERE 1=1 ".$strwhere." ORDER BY `tbl_location`.`name` DESC LIMIT $start,$leng";
            $objdata->Query($sql);
            while($rows=$objdata->Fetch_Assoc()){
                ...
                echo "<a href='?com=location&task=list-lichsu&loc=$ids' class='btn btn-default'>Lịch sử</a>&nbsp";
                echo "<a href='?com=location&task=list-vanhoa&loc=$ids' class='btn btn-default'>Văn hóa</a>&nbsp";
                echo "<a href='?com=location&task=list-lehoi&loc=$ids' class='btn btn-default'>Lễ hội</a>";
                echo "</td>";
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="Footer_list">
        <tr>
            <td align="center">
                <?php
                paging($total_rows,MAX_ROWS,$cur_page,'?com=location&page={page}');
                ?>
            </td>
        </tr>
    </table>
