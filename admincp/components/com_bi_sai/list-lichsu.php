...
define('OBJ_PAGE','POSITION_HISTORY');
define('TASK_NAME', ' Danh sách bài viết về lịch sử');
write_path();
...
if($position_id!=''){
    ...
    $sql="SELECT count(*)  as num FROM `tbl_location_content_history` WHERE 1=1 ".$strwhere;
    $objdata->Query($sql);
    ...
    <hr/>
    <div id="list">
        <h3>Lịch sử của <?php echo $row_loc['name'] ?></h3>
        <form id="frm_list" name="frm_list" method="post" action="">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="Header_list">
                <tr>
                    ...
                    <td align="right">
                        <a href="?com=position&task=add_history&pos=<?php echo $position_id; ?>" class="btn btn-success button"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
                    </td>
                </tr>
            </table>
            ...
            <table class="table table-bordered">
                ...
                $leng=MAX_ROWS;
                $sql="SELECT * FROM `tbl_location_content_history` WHERE 1=1 ".$strwhere." ORDER BY `name` DESC LIMIT $start,$leng";
                $objdata->Query($sql);
                while($rows=$objdata->Fetch_Assoc()){
                    ...
                    echo "<a href='index.php?com=".COMS."&amp;task=active_history&amp;id=$ids&pos=$position_id'>";
                    showIconFun('publish',$rows["isactive"]);
                    echo "</a>";
                    echo "</td>";

                    echo "<td  align='center'>";
                    echo "<a href='index.php?com=".COMS."&amp;task=edit_history&amp;id=$ids&pos=$position_id'>";
                    showIconFun('edit','');
                    echo "</a>";
                    echo "</td>";

                    echo "<td align='center'>";
                    echo "<a href='index.php?com=".COMS."&amp;task=delete_history&amp;id=$ids&pos=$position_id' onclick=\"return confirm('Bạn có chắc muốn xóa !')\">";
                    showIconFun('delete','');
                    echo "</a>";
                    echo "</td>";
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="Footer_list">
            <tr>
                <td align="center">
                    <?php
                    paging($total_rows,MAX_ROWS,$cur_page,'?com=position&task=list-lichsu&page={page}');
                    ?>
                </td>
            </tr>
        </table>
