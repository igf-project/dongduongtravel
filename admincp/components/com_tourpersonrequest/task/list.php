...
// Gán strwhere
if($keyword!='')
	$strwhere.=" AND ( `tbl_tour_person_request`.`fullname` like '%$keyword%' )";
if($action!='' && $action!='all')
	$strwhere.=" AND ( `tbl_tour_person_request`.`type` = $action )";

$sql="SELECT count(*)  as num FROM `tbl_tour_person_request` WHERE 1=1 ".$strwhere;
$objdata->Query($sql);
	...
	<form id="frm_list" name="frm_list" method="post" action="">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="Header_list">
			<tr>
				...
				<td align="right">
					<a href="?com=tourpersonrequest&task=add" class="btn btn-success button"><i class="fa fa-plus" aria-hidden="true"></i> Thêm mới</a>
				</td>
			</tr>
		</table>
		...
		<table class="table table-bordered">
			<tr class="header">
				...
				<th align="center">Điểm đón</th>
				<th align="center">Điểm đến</th>
				<th class="text-center">SĐT</th>
				<th align="center">Email</th>
				<th align="center">Thời gian đi</th>
			...
			$leng=MAX_ROWS;
			$sql="SELECT * FROM tbl_tour_person_request WHERE 1=1 ".$strwhere." ORDER BY `date` DESC LIMIT $start,$leng";
			$objdata->Query($sql);
			while($rows=$objdata->Fetch_Assoc()){
				$ids=$rows['id'];$start++;
				$name = stripcslashes($rows['fullname']);
				$where_start = stripcslashes($rows['where_start']);
				$position = stripcslashes($rows['position']);
				$phone = stripcslashes($rows['phone']);
				$email = stripcslashes($rows['email']);
				$time = date('d/m-Y', strtotime($rows['time_start']));
				$type = $rows['type'];
				$icon_type='';
				switch ($type) {
					case '0':
					$icon_type='<img src="'.ROOTHOST_ADMIN.'images/new.gif">';
					break;
					case '1':
					$icon_type='<i class="fa fa-check-circle" style="color:green" aria-hidden="true"></i>';
					break;
					default:
					$icon_type='<i class="fa fa-times-circle" style="color:red" aria-hidden="true"></i>';
					break;
				}
				echo "<tr name='trow'>";
